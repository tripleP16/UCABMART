...
$Reporte = $_SERVER['DOCUMENT_ROOT'] . '//UCABMART//reportes//ReporteCarnetNatural.jrxml';
...
/*--------------------------------------------------------------------------------------------------------------
        CODIGO SI JUNTAMOS LOS DOS REPORTES EN UNO
...
if($VARIABLE-QUE-VIENE-DEL-CONTROLLER == 'NATURAL'){
    //Ruta del reporte compilado Jasper generado por IReports
    $Reporte = $_SERVER['DOCUMENT_ROOT'] . '//UCABMART//reportes//ReporteCarnetNatural.jrxml';
    //Funcion de conexion a mi base de datos tipo MySql
    $sql = "SELECT per_nat_cedula, per_nat_primer_nombre, per_nat_segundo_nombre , per_nat_primer_apellido, per_nat_segundo_apellido, lug_nombre,FK_tie_codigo FROM ucabmart.persona_natural JOIN ucabmart.tienda ON fk_tie_codigo = tie_codigo JOIN ucabmart.lugar ON tienda.FK_lug_codigo = lug_codigo WHERE per_nat_cedula = '".$persona."'";
    $reporteNombreFile="Reporte_Carnet_Natural";
}
else($VARIABLE-QUE-VIENE-DEL-CONTROLLER == 'JURIDICA'){
    //Ruta del reporte compilado Jasper generado por IReports
    $Reporte = $_SERVER['DOCUMENT_ROOT'] . '//UCABMART//reportes//ReporteCarnetJuridica.jrxml';
    //Funcion de conexion a mi base de datos tipo MySql
    $sql = "SELECT per_jur_rif, per_jur_razon_social, per_jur_denominacion_comercial, lug_nombre,FK_tie_codigo FROM ucabmart.persona_juridica JOIN ucabmart.tienda ON FK_tie_codigo = tie_codigo JOIN ucabmart.lugar ON tienda.FK_lug_codigo = lug_codigo WHERE per_jur_rif = '".$personajuridica."'";
    $reporteNombreFile="Reporte_Carnet_Juridica";
}

//Declarar nombre del fichero
$filename = $reporteNombreFile. date("Y-m-d")."_". $fecha_partir3.'_'. $fecha_partir2.'_'. $fecha_partir4.'.pdf';
//Ruta a donde deseo guardar mi archivo de salida pdf 
$SalidaReporte=  $_SERVER['DOCUMENT_ROOT'] . '//UCABMART//salidareportes//'.$filename;
//Parametro en caso de que el reporte no este parametrizado
$Parametro=new java("java.util.HashMap");
